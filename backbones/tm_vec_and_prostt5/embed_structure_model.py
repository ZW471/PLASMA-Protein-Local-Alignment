...
class trans_basic_block(pl.LightningModule):
    ...
    def forward(self, x, src_mask, src_key_padding_mask, include_residue_level_out=False):
        x_no_pooling = None
        x = self.encoder(x, mask=src_mask, src_key_padding_mask=src_key_padding_mask)
        if include_residue_level_out:
            x_no_pooling = x.clone()
        lens = torch.logical_not(src_key_padding_mask).sum(dim=1).float()
        x = x.sum(dim=1) / lens.unsqueeze(1)
        x = self.dropout(x)
        x = self.mlp(x)
        return x if x_no_pooling is None else (x_no_pooling, x)
